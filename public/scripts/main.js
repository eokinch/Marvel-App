...
app.characters = [{
	...
	id: 1009707,
	name: 'Wasp',
	facts: [["I used to be a Fashion Designer and TV host"], ["I was married to Hank Pym"], ["Superpowers: Flight and Size Reduction"], ["My Initals are: JvD"], ["First Appearance: Tales to Astonish #44"], ["Group Affiliations: Avengers"]],
	first: 11387
}, {
	id: 1010338,
	name: 'Captain Marvel',
	...
	id: 1009189,
	name: 'Black Widow',
	facts: [["I was married to a Soviet Spy"], ["I was born in 1928"], ["Superpowers: world class athlete, gymnast, acrobat, aerialist, advanced healing capabilities and resistant to aging"], ["My Initals are: NR"], ["First Appearance: Tales of Suspense #52 (1964)"], ["Group Affiliations: Avengers"]],
	first: 11295
}, {
	id: 1009356,
	name: 'Human Torch',
	...
	id: 1009471,
	name: 'Nick Fury',
	facts: [["I fought in World War 2 with Captain America and my Howling Commandos"], ["I lost my eye in a grenade accident"], ["Superpowers: ages very slowly"], ["My Initals are: NF"], ["First Appearance: Sgt. Fury and His Howling Commandos #1 (1963)"], ["Group Affiliations: S.H.I.E.L.D."]],
	first: 10644
}, {
	id: 1010740,
	name: 'Winter Soldier',
	facts: [["I am often kept in cryogenic stasis "], ["I once had a thing with Black Widow"], ["Superpowers: superhuman strength, speed, reflexes, healing, stamina, agility and semi-immortality"], ["My Initals are: JBB"], ["First Appearance: Captain America #1 (2004)"], ["Group Affiliations: HYDRA"]],
	first: 91
}, {
	id: 1009524,
	name: 'Quicksilver',
...
app.parseData = function () {
	...
	chosen.forEach(function (item, index) {
		...
		var guessSubmit = $("<button class=\"guess-submit\" data-number=\"" + number + "\" data-id=\"" + item.id + "\">").addClass("guess-submit-" + number).text('Guess');
		var guessDiv = $('<div class="guess-container">').append(guess, guessLabel, guessSubmit);
...
app.handleGuess = function () {
	$('.guess-submit').on('click', function (e) {
		...
		var characterCardId = $(this).data('id');
...
app.checkCorrect = function () {
	...
	if (successfulCards.length === 3) {
		...
		var winnerScore = Math.floor(app.guessCount / 6 * 100);
		var winnerData = {
			name: app.username,
			score: winnerScore
		};
		...
		dbRef.on('value', function (snapshot) {
			var allWinners = snapshot.val();
			for (var key in allWinners) {
				var _winnerData = allWinners[key];
				var winner = {
					name: _winnerData.name,
					score: _winnerData.score
				};
				winners.push(winner);
			}
			var winnersSort1 = _.sortBy(winners, 'score');
			var winnersSorted = winnersSort1.reverse();
			if (winnersSorted.length >= 6) {
				var first6 = _.first(winnersSorted, [6]);
				first6.forEach(function (x) {
					var name = $('<p>').addClass('name').text(x.name);
					var score = $('<p>').addClass('score').text(x.score);
					var winner = $('<div>').addClass('winner-data').append(name, score);
					$('.leader-board').append(winner);
				});
			} else {
				winnersSorted.forEach(function (x) {
					var name = $('<p>').addClass('name').text(x.name);
					var score = $('<p>').addClass('score').text(x.score);
					var winner = $('<div>').addClass('winner-data').append(name, score);
					$('.leader-board').append(winner);
...
app.checkCount = function () {
	if (app.guessCount === 0) {
		...
		var losingCardIds = $(".character-card").map(function () {
			var id = $(this).data('id');
			ids.push(id);
		});
		...
		ids.forEach(function (id) {
			var findById = _.find(app.characters, function (x) {
				var characterId = x.id;
				if (id === characterId) {
					return x;
				}
			});
			var image = $('<img>').attr('src', findById.image + ".jpg");
			var name = $('<p>').text(findById.name);
			var unguessedDiv = $('<div>').append(image, name);
